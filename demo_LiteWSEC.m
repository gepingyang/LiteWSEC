...
s =5400; %the number of data points that are used to generate prototypes and prototype graph.
p = 500; % the number of prototypes
r =5; %the number of nearest prototypes.
k =10; % the number of clusters.
M = 20; % the number of emsemble
lowK = 20; %\tau_min
upK = 60;   %\tau_max
seed.end = 10;
seed.start = 1;
...
for i = seed.start : seed.end 
    rand('seed',i);    
     fprintf('Seed No: %d\n',i);
     tic;
     [label, label_orig] = LiteWSEC(dataname, s, p, M, r, lowK, upK,  k);
     time_once = toc;
